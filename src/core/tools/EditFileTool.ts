...
export class EditFileTool extends BaseTool<"edit_file"> {
	...
	async execute(params: EditFileParams, task: Task, callbacks: ToolCallbacks): Promise<void> {
		...
		let new_string = typeof params.new_string === "string" ? params.new_string : ""
		if (typeof params.new_string !== "string" && params.new_string !== undefined) {
			console.warn(`[EditFileTool] converting incorrect model output ${typeof params.new_string} to string`)
			new_string = JSON.stringify(params.new_string, null, "\t")
		}
		// kilocode_change end
